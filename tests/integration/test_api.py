...
# ── Test API key — must match what the app sees at test time ──
TEST_API_KEY = "voyage-dev-key-2024"
...
def build_mock_model_cache():
    ...
    # Mock gender model — 6 features (multicollinear ones removed)
...
@pytest.fixture
def client():
    """Test client with mocked model cache and fixed API key."""
    mock_cache = build_mock_model_cache()

    with (
        patch("api.main.MODEL_CACHE", mock_cache),
        ...
        # Re-import inside patch so API_KEY is picked up fresh
        import importlib

        import api.main as main_module

        importlib.reload(main_module)
        main_module.API_KEY = TEST_API_KEY  # force-set directly as fallback
...
class TestGenderEndpoint:
    # 6-feature payload matching the updated model schema
    PAYLOAD = {
        "age": 34,
        "travel_frequency": 8,
...
class TestRecommendEndpoint:
    ...
    def test_top_n_respected(self, client):
        resp = client.post("/recommend/hotels", json={"user_code": 1, "top_n": 11}, headers=HEADERS)

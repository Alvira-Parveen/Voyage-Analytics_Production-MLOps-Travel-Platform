...
FEATURE_COLS = [
    "age", "company_enc",
    "travel_frequency", "avg_flight_price", "total_flight_spend",
    "preferred_flight_type_enc", "hotel_bookings", "avg_hotel_spend",
    "age_group_enc", "spending_category_enc",
]
...
def train_gender_classifier(data_dir: str = "data/processed", version: str = "1.0"):
    ...
    for name, model in CANDIDATE_MODELS.items():
        with mlflow.start_run(run_name=name):
            ...
            mlflow.log_params({"model": name, "features": len(FEATURE_COLS),
            ...
            results[name] = {**metrics, "run_id": run_id, "exp_id": exp_id,
                             "model": model,
                             "scaler": scaler if use_scaled else None}

            print(f"  {'✅' if metrics['f1_macro'] > 0.75 else '➡️ '}  {name:<22}"
    ...
    artifact = {"model": best["model"], "scaler": best["scaler"],
    ...
    save_model_metadata(
        ...
        version=version,
        algorithm=best_name,
        metrics={"accuracy": best["accuracy"], "f1_macro": best["f1_macro"],
